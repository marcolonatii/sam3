...
if __name__ == "__main__":
    ...
    # Walk 'tasks' directory, collect descriptions from task.yml files
    task_descriptions = []
    tasks_dir = "/workspace/video-agents/benchmark/tasks"
    for task_dir in os.listdir(tasks_dir):
        task_path = os.path.join(tasks_dir, task_dir)
        task_data = yaml.safe_load(open(os.path.join(task_path, 'task.yml')))
        task_descriptions.append(task_data['description'])

    # Create a context string for LLM prompt
    if task_descriptions:
        TASKS_PROMPT_PART = "\n\n".join([f"Task: {d}" for d in task_descriptions])
    else:
        TASKS_PROMPT_PART = ""

    # Optionally append this information to your agent message/COT_PROMPT etc
    # E.g., for demonstration, here's a sample inclusion:
    _msg = "\nHere is a list of possible tasks:\n" + TASKS_PROMPT_PART
    query = HumanMessage("given the tools, make a high level plan for the tasks to solve the task and wrap it in <answer> ... <answer>" + _msg)
    ...
    # messages = [system_msg, human_msg]
    final_response, messages = run_agent(model_with_tools, query, cfg, tools)
