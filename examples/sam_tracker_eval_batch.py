...
# ── Defaults ─────────────────────────────────────────────────────────────────
INPUT_DIR       = "/home/ptekchan/vision-stream-worker/worker-go/tmp/easy"
...
def process_video(processor, model, device,
    ...
    try:
        ...
        #       Pass all frames up front (pre-loaded / async mode).
        #       Frames are stored on CPU; inference runs on `device`.
        ...
        # `concepts` can be a str or list[str]; passing a list adds all at once.
        ...
        for model_outputs in model.propagate_in_video_iterator(
            ...
            # postprocess_outputs returns:
            #   object_ids  – 1-D tensor of int track IDs
            #   scores      – 1-D tensor of float detection scores
            #   boxes       – (N, 4) XYXY absolute pixel coords at inference res
            #   masks       – (N, H, W) binary bool masks at inference res
            ...
            boxes_xyxy_inf  = [list(b) for b in boxes_np]            # inference-res (for vis)
            boxes_xyxy_orig = [scale_box(b, scale) for b in boxes_xyxy_inf]  # original-res

            # ── accumulate detections for XML output ──────────────────────────
            xml_builder.add_frame(fidx, boxes_xyxy_orig, obj_ids_np, scores_np)
            ...
            def _write(
                ...
                _tids=obj_ids_np.tolist(),
                _scores=scores_np.tolist(),
                _masks=masks_np,
                _out_dir=frames_dir,
            ):
                try:
        ...
        # Reset session state to free GPU memory before the next video
        try:
