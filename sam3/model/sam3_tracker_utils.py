...
# Try to import triton-based EDT, fallback to OpenCV for CPU
try:
...
def edt_cpu_fallback(data: torch.Tensor):
    """CPU fallback for EDT using OpenCV"""
    assert data.dim() == 3
    B, H, W = data.shape
    ...
    for b in range(B):
        ...
        dt = cv2.distanceTransform(mask, cv2.DIST_L2, 0)
        output[b] = dt
    return torch.from_numpy(output).to(data.device)

def edt(data: torch.Tensor):
    """EDT function that works on both CPU and CUDA"""
    if data.is_cuda and _has_triton:
        return edt_triton(data)
    else:
